...
(function() {
    ...
    // Talk to Human - Computer Form Toggle (contact page specific)
    document.addEventListener('DOMContentLoaded', function() {
        var showComputerFormBtn = document.getElementById('showComputerForm');
        var backToOptionsBtn = document.getElementById('backToOptions');
        var computerForm = document.getElementById('computerForm');
        var talkHumanOptions = document.getElementById('talkHumanOptions');
        var talkHumanForm = document.getElementById('talkHumanForm');

        // Store original form HTML for reset
        var originalFormHTML = computerForm ? computerForm.innerHTML : '';

        if (showComputerFormBtn && computerForm && talkHumanOptions) {
            showComputerFormBtn.addEventListener('click', function() {
        ...
        // Form submission
        if (talkHumanForm) {
            talkHumanForm.addEventListener('submit', function(e) {
                e.preventDefault();
                var formData = new FormData(talkHumanForm);
                var submitBtn = talkHumanForm.querySelector('.talk-human-send');
                ...
                fetch('https://api.web3forms.com/submit', {
                    ...
                    body: JSON.stringify({
                        access_key: '1b112abb-70d7-4e79-a0af-5d0b3e425015',
                        name: formData.get('name'),
                        phone: formData.get('phone'),
                        message: formData.get('message'),
                        subject: 'Talk to Human - SMS Request'
                    })
                })
                .then(function(response) {
                    if (response.ok) {
                        computerForm.innerHTML = '<div style="text-align: center; padding: 20px 0;">' +
                            ...
                            '<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3"><polyline points="20 6 9 17 4 12"></polyline></svg>' +
                            '</div>' +
        ...
        // Store reset function for modal close
        window._resetContactForm = function() {
